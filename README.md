...
- the internal `PipeWireAO.LibPipeWire` module is the thin, unsafe C ABI layer
...
profiler samples,
typed core protocol events and methods, streams, multi-port filters,
standalone ndarray filters, owned SPA PODs for raw-audio and raw-video formats,
and mapped buffers.
...
The generated `PipeWireAO.LibPipeWire` module is an unexported implementation
detail, not a supported client API. Missing native coverage should be added to
the generated layer and exposed through an owning, idiomatic `PipeWireAO`
wrapper. `SPA.Pointer` is intentionally borrowed and does not keep its pointee
...
CPU capabilities. PipeWireAO 0.6.7 requires PipeWireAO_jll 1.7.0+14 or newer.
